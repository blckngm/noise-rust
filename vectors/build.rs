...
fn gen<O: Write>(mut out: O) -> ::std::io::Result<()> {
    ...
    hashes.insert("SHA256", vec!["crypto::Sha256", "ring::Sha256", "sodium::Sha256"]);
    hashes.insert("SHA512", vec!["crypto::Sha512", "ring::Sha512", "sodium::Sha512"]);
    ...
    writeln!(out, "    match (v.dh.clone().as_ref(), v.cipher.clone().as_ref(), v.hash.clone().as_ref()) {{")?;
                ...
                for hh in hashes.get(h).unwrap() {
                    for cc in ciphers.get(c).unwrap() {
                        for dd in dhs.get(d).unwrap() {
                            writeln!(out, "            verify_vector_with::<{}, {}, {}>(&v);", dd, cc, hh)?;
                        }
                    }
                }
                writeln!(out, "        }}")?;
